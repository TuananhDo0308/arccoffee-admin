...
const EditEmployeeModal: React.FC<EditEmployeeModalProps> = ({ isOpen, employee, onClose, onSuccess }) => {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (regionsResponse.data && regionsResponse.data.data) {
        ...
        // Set initial cities and districts based on current employee data
        const currentRegion = regionsData.find((region: Region) => region.id === employee.regionId)
        if (currentRegion) {
          setCities(currentRegion.cities)
          const currentCity = currentRegion.cities.find((city: City) => city.id === employee.cityId)
          if (currentCity) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await httpClient.put({
        url: `${apiLinks.employee.index}/${formData.id}`,
        data: updateData,
        token: token,
      })
      onSuccess()
